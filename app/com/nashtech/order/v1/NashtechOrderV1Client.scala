  ...
  class Client(
    ...
    object Orders extends Orders {
      ...
      override def getAll(
        merchantId: String,
        requestHeaders: Seq[(String, String)] = Nil
      )(implicit ec: scala.concurrent.ExecutionContext): scala.concurrent.Future[Seq[com.nashtech.order.v1.models.Order]] = {
        _executeRequest("GET", s"/${play.utils.UriEncoding.encodePathSegment(merchantId, "UTF-8")}/orders/all", requestHeaders = requestHeaders).map {
          case r if r.status == 200 => _root_.com.nashtech.order.v1.Client.parseJson("Seq[com.nashtech.order.v1.models.Order]", r, _.validate[Seq[com.nashtech.order.v1.models.Order]])
          case r if r.status == 401 => throw com.nashtech.order.v1.errors.UnitResponse(r.status)
          case r if r.status == 404 => throw com.nashtech.order.v1.errors.UnitResponse(r.status)
          case r => throw com.nashtech.order.v1.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 200, 401, 404")
        }
